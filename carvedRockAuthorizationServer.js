...
var clients = [
    {
        ...
        client_secret: "globomantics-client-secret-1", // Never store it in here
...
app.get("/authorize", function (req, res) {
    // Retrieving the client id from the request.query (from the build url method remember)
    ...
    if (!client) {
        ...
        return;
        // If there is a client but the redirect uri given is not in the redirect uri list of the client then -> provided redirect uri is invalid
        // We make sure that the redirect uris always match to avoid tricky business
        ...
        return;
        // Else the client exists and is valid -> move on
    } else {
        // If there is a request.query.scope then take it and split it with " "
        // The split() method returns the new array.
        var request_scope = req.query.scope ? req.query.scope.split(" ") : undefined;
        ...
        // res.render("approve", { client: client, reqid: reqid, scope: request_scope });
        return;
    }
});
